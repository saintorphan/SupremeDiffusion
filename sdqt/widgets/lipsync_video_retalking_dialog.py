"""Video Retalking lip sync dialog — popup from video player context menu."""
...
class LipSyncVideoRetalkingDialog(QDialog):
    """Modal dialog for lip-syncing a video via Video Retalking."""
    ...
    def __init__(self, source_path: str, state=None, parent=None) -> None:
        ...
        self.setWindowTitle("Lip Sync \u2014 Video Retalking")
    ...
    def _build_ui(self) -> None:
        ...
        s1 = QHBoxLayout()
        s1.setSpacing(6)
        s1.addWidget(QLabel("Expression:"))
        self._expression = QComboBox()
        self._expression.addItems(["neutral", "smile"])
        self._expression.setFixedWidth(90)
        s1.addWidget(self._expression)
        s1.addWidget(QLabel("Upper Face:"))
        self._up_face = QComboBox()
        self._up_face.addItems(["original", "surprise", "angry"])
        self._up_face.setFixedWidth(100)
        s1.addWidget(self._up_face)
        s1.addWidget(QLabel("Img Size:"))
        self._img_size = QSpinBox()
        self._img_size.setRange(256, 512)
        self._img_size.setValue(384)
        self._img_size.setFixedWidth(65)
        s1.addWidget(self._img_size)
        self._nosmooth = QCheckBox("No Smooth")
        s1.addWidget(self._nosmooth)
        s1.addStretch()
        cl.addLayout(s1)

        # Settings row 2 (padding)
        s2 = QHBoxLayout()
        s2.setSpacing(6)
        for label_text, attr_name, default in [
            ("Pad T:", "_pad_top", 0),
            ("B:", "_pad_bottom", 0),
            ("L:", "_pad_left", 0),
            ("R:", "_pad_right", 0),
        ]:
            s2.addWidget(QLabel(label_text))
            spin = QSpinBox()
            spin.setRange(0, 50)
    ...
    def _on_run(self) -> None:
        ...
        import tempfile
        tmp = tempfile.NamedTemporaryFile(suffix=".mp4", delete=False, prefix="vr_")
        tmp.close()
        output = tmp.name

        self._go_btn.setVisible(False)
        self._abort_btn.setVisible(True)
        self._status.setText("Running Video Retalking...")
        if self.state:
            self.state.unload_pipelines()

        from sdqt.workers.video_retalking import VideoRetalkingWorker
        worker = VideoRetalkingWorker(
            video_path=self._source_path,
            audio_path=audio,
            face_bbox=self._selected_bbox,
            output_path=output,
            expression=self._expression.currentText(),
            up_face=self._up_face.currentText(),
            img_size=self._img_size.value(),
            nosmooth=self._nosmooth.isChecked(),
            padding=[
                self._pad_top.value(),
                self._pad_bottom.value(),
                self._pad_left.value(),
                self._pad_right.value(),
            ],
            parent=self,
        )
