"""AudioGen tab — text-to-sound-effect generation using Meta AudioCraft."""
...
from sdqt.workers.audiogen import AudioGenWorker
...
class AudioGenTab(BaseTab):
    """Sound effect generation using AudioGen."""

    def __init__(self, state: AppState, parent=None) -> None:
        super().__init__(state, parent)
        self._worker: AudioGenWorker | None = None
    ...
    def _on_done(self, result: str) -> None:
        ...
        logger.info("AudioGen complete: %s", result)
    ...
    def _on_error(self, msg: str) -> None:
        ...
        self._abort_btn.setVisible(False)
        self.state._force_gc()
        logger.error("AudioGen error: %s", msg)
    ...
    def _save_to_config(self) -> None:
        ...
        try:
            ...
            return
        cfg.audiogen_prompt = self._prompt.toPlainText()
        cfg.audiogen_duration = self._duration.value()
        cfg.audiogen_temperature = self._temperature.value()
        cfg.audiogen_top_k = self._top_k.value()
        cfg.audiogen_cfg = self._cfg.value()
    ...
    def _restore_from_config(self) -> None:
        ...
        try:
            ...
            return
        prompt = getattr(cfg, "audiogen_prompt", "") or ""
        if prompt:
            self._prompt.setPlainText(prompt)
        else:
            self._prompt.clear()
        self._duration.setValue(getattr(cfg, "audiogen_duration", 10.0))
        self._temperature.setValue(getattr(cfg, "audiogen_temperature", 1.0))
        self._top_k.setValue(getattr(cfg, "audiogen_top_k", 250))
        self._cfg.setValue(getattr(cfg, "audiogen_cfg", 3.5))
